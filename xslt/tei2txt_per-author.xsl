...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
       <!-- 
           Example of command java -jar ../../../../saxon.jar -s:18th/per-sonnet -o:../test/18th/per-author -xsl:../../xslt/tei2txt_per-author.xsl-->
        
        <xsl:variable name="fileName" select="document-uri(current()) => substring-after($dir) => substring-before('xml')"/>
        <xsl:variable name="authorFolder" select="document-uri(current())  => substring-after($dir) => substring-before('_')"/>
        <xsl:result-document href="{$authorFolder}/{$fileName}txt">
